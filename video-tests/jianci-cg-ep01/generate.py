...
def main():
 cfg=json.loads((ROOT/'episode.json').read_text());assert len(cfg['clips'])==6
 if os.environ.get('JIANCI_CG_EP01_EXECUTE')!='1': print('Validated six 10-second clips, 480P, estimate USD3; no API calls.');return
 if os.environ.get('GITHUB_RUN_ATTEMPT','1')!='1':raise RuntimeError('No automatic rerun allowed')
 out=Path('output/jianci-cg-ep01');out.mkdir(parents=True,exist_ok=True)
 ...
 base='https://raw.githubusercontent.com/'+os.environ['GITHUB_REPOSITORY']+'/'+os.environ['GITHUB_SHA']+'/video-tests/jianci-cg-ep01/'
 report={'episode':1,'version':'CG v1','model':'wan3.0-video','estimated_usd':3,'actual_billing':'provider history','clips':[]};s.save_report(out,report)
 pending=[]
 for clip in cfg['clips']:
  row={'id':clip['id'],'state':'SUBMITTING'};report['clips'].append(row);s.save_report(out,report)
  try:
   result=gate.request('/v2/wan3.0-video',{'prompt':clip['prompt'],'reference_images':[base+'cast.jpg',base+'room.jpg',base+'jianci-approved.jpg'],'duration':10,'resolution':'480P','aspect_ratio':'16:9','audio':True,'prompt_extend':False,'enable_thinking':True,'seed':910601,'negative_prompt':'stairs, staircase, split screen, contact sheet, labels, text, captions, live action, flat 2D outlines, chibi, glowing eyes, exaggerated gore, repeated turning, repeated speech, additional dialogue, music, opaque black scene'})
 ...
 s.save_report(out,report)
 if sum('media' in x for x in report['clips'])!=6:raise SystemExit(2)
