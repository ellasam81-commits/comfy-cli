...
def config():
    ...
    assert len(cfg["clips"]) == 8 and [c["id"] for c in cfg["clips"]] == ["01","03","04","05","06","08","10","11"]
...
def main():
    cfg = config()
    if os.environ.get("JIANCI_EP04_PATCH_EXECUTE") != "1":
        print("Validated: Episode 04 repair only, 8 x 5 sec, 12 planned shots, estimated USD 2. No API calls.")
        return
    if int(os.environ.get("GITHUB_RUN_ATTEMPT", "1")) != 1:
        raise RuntimeError("No automatic rerun: could duplicate paid submissions.")
    out = Path("output/jianci-episode04-patch")
    ...
    report = {"episode": 4, "model": "wan3.0-video", "estimated_usd": 2, "clips": []}
    ...
    print(f"Downloaded {count}/8 clips. No retries submitted.")
    if count != 8:
        raise SystemExit(2)
