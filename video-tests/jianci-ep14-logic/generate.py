...
OUT = Path('output/jianci-ep14-logic')
...
def main():
    ...
    if os.environ.get('JIANCI_EP14_EXECUTE') != '1':
        print('Dry run: two logic corrections, 20 seconds, 480P, USD 0.588 published estimate, no requests')
        return
    ...
    report = {'episode': 14, 'provider': 'XRToken', 'base_url': host, 'model': cfg['model'],
              'estimate_usd': 0.588, 'max_output_seconds': 20, 'clips': []}
    ...
    save()
    jian_media = known_media('14c89ad0-6f32-49d6-9306-790b7f412dc5', 'jian14.mp4')
    doctor_media = known_media('b322ef03-525d-4827-a269-07c4d88f82fe', 'doctor14.mp4')
    jian = reference(jian_media, 2, 'jian14')
    doctor = reference(doctor_media, 2, 'doctor14')
    if not submit(cfg['clips'][2], [jian]):
        raise SystemExit(2)
    if not submit(cfg['clips'][3], [doctor]):
        raise SystemExit(2)
    if not poll(report['clips']):
        raise SystemExit(2)
    print('Two requested logic corrections returned; review required', flush=True)
