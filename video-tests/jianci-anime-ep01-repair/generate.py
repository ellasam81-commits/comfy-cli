...
def main():
 cfg=json.loads((ROOT/'episode.json').read_text());assert len(cfg['clips'])==2
 ...
 out=Path('output/jianci-anime-ep01-repair');out.mkdir(parents=True,exist_ok=True)
 ...
 report={'version':'2D anime remake v1','duration':60,'estimate_usd':0.5,'clips':[]};s.save_report(out,report);pending=[]
 for c in cfg['clips']:
 ...
 s.save_report(out,report)
 if sum('media' in r for r in report['clips'])!=2:raise SystemExit(2)
