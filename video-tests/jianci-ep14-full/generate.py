...
OUT = Path('output/jianci-ep14-full')


def main():
    cfg = json.loads((ROOT / 'episode.json').read_text())
    assert cfg['episode'] == 14 and len(cfg['clips']) == 7
    assert sum(c['duration'] for c in cfg['clips']) == 70
    assert cfg['resolution'] == '480P' and cfg['model'] == 'wan3.0-video'
    if os.environ.get('JIANCI_EP14_EXECUTE') != '1':
        print('Dry run: two completion scenes, 20 seconds, 480P, USD 0.588 published estimate, no requests')
        return
    ...
    save()
    mother_media = known_media('5647b28a-ca4f-4987-b925-3f09c5afd4cf', 'mother14.mp4')
    mother = reference(mother_media, 8.5, 'mother14')
    if not submit(cfg['clips'][1], [cfg['reference_url']]):
        raise SystemExit(2)
    if not submit(cfg['clips'][6], [mother]):
        raise SystemExit(2)
    if not poll(report['clips']):
        raise SystemExit(2)
    print('Two completion scenes returned; review required', flush=True)
