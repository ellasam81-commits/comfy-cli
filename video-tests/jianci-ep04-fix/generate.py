...
def main():
 cfg=json.loads((ROOT/'episode.json').read_text());assert len(cfg['clips'])==1 and cfg['episode']==4
 if os.environ.get('JIANCI_EP04_EXECUTE')!='1':print('6 x10 seconds, USD3 base estimate; no API calls');return
 if os.environ.get('GITHUB_RUN_ATTEMPT','1')!='1':raise RuntimeError('No duplicate paid rerun')
 out=Path('output/jianci-ep04-fix');out.mkdir(parents=True,exist_ok=True)
 ...
 report={'episode':4,'model':'wan3.0-video','estimate_usd':0.5,'mode':'only clip05 exterior continuity correction','clips':[]};s.save_report(out,report);pending=[]
 starts=['scene-05.jpg','scene-05.jpg','scene-05.jpg','scene-01.jpg','scene-01.jpg','scene-01.jpg']
 refs=[['scene-06.jpg'],['scene-06.jpg'],[],['scene-05.jpg'],['scene-05.jpg'],['scene-05.jpg']]
 for c in cfg['clips']:
  i=int(c['id'])-1
