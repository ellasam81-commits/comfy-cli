...
def main():
 cfg=json.loads((ROOT/'episode.json').read_text());assert len(cfg['clips'])==6 and cfg['episode']==3
 ...
 out=Path('output/jianci-ep03-evidence');out.mkdir(parents=True,exist_ok=True)
 ...
 report={'episode':3,'model':'wan3.0-video','estimate_usd':3,'mode':'six new episode3 scenes','clips':[]};s.save_report(out,report);pending=[]
