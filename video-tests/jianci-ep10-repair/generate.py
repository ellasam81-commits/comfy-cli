...
def main():
 cfg=json.loads((ROOT/'episode.json').read_text());assert len(cfg['clips'])==1 and cfg['episode']==10
 if os.environ.get('JIANCI_EP10_EXECUTE')!='1':print('6 x10 seconds, USD3 base estimate; no API calls');return
 if os.environ.get('GITHUB_RUN_ATTEMPT','1')!='1':raise RuntimeError('No duplicate paid rerun')
 out=Path('output/jianci-ep10-repair');out.mkdir(parents=True,exist_ok=True)
 ...
 prior=gate.request('/v2/requests/f59ad346120ee075f4b133e724b0986c/status')
 if prior.get('status')!='FAILED':raise RuntimeError('Prior request is not confirmed failed; refusing another submission')
 ...
 s.save_report(out,report)
 if sum('media' in c for c in report['clips'])!=1:raise SystemExit(2)
