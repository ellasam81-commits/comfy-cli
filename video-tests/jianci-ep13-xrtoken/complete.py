...
OUT = Path('output/jianci-ep13-xrtoken-completion')
...
def main():
    ...
    report = {'episode': 13, 'provider': 'XRToken', 'base_url': host, 'model': cfg['model'],
              'estimate_usd': 0.882, 'max_output_seconds': 30, 'clips': []}
    ...
    save()
    # Only these missing Episode 13 clips may be submitted, once each.
    if not submit(cfg['clips'][2]) or not poll([report['clips'][-1]]):
        raise SystemExit(2)
    # Recover this episode's already purchased mother shot, by exact known ID.
    res = request('/v1/videos/generations/cd8fc994-8cfb-447e-9b6a-554b742979f1')
    if res.get('status') != 'succeeded':
        raise RuntimeError('Episode 13 shot 07 is not available')
    content = res.get('content') or {}
    url = res.get('video_url') or content.get('video_url')
    if not url or urllib.parse.urlparse(url).scheme != 'https':
        raise RuntimeError('Missing HTTPS episode 13 shot 07 URL')
    with urllib.request.urlopen(url, timeout=120) as media:
        (OUT / '07-raw.mp4').write_bytes(media.read())
    ...
    if not submit(cfg['clips'][8], frame_ref('08')) or not poll([report['clips'][-1]]):
        raise SystemExit(2)
    print('Episode 13 missing clips completed; no successful clips regenerated', flush=True)
