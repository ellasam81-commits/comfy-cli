"""One authorized episode-six production; twelve submissions, no retries. References stay transient."""
...
def config():
    ...
    assert cfg["episode"] == 6 and cfg["duration_seconds"] == 60 and cfg["shots"] == 12
...
def main():
    cfg = config()
    if os.environ.get("JIANCI_EP06_EXECUTE") != "1":
        print("Validated: Episode 06, 12 x 5 sec, 12 planned shots, estimated USD 3. No API calls.")
        return
    if int(os.environ.get("GITHUB_RUN_ATTEMPT", "1")) != 1:
        raise RuntimeError("No automatic rerun: could duplicate paid submissions.")
    out = Path("output/jianci-episode06")
    ...
    for role, name, position, crop in [
        ...
        ("scene","03-raw.mp4",6.0,"scale=832:480"),
    ]:
    ...
    report = {"episode": 6, "model": "wan3.0-video", "estimated_usd": 3, "clips": []}
