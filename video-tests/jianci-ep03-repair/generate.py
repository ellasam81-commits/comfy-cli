...
def main():
 cfg=json.loads((ROOT/'episode.json').read_text());assert len(cfg['clips'])==2 and cfg['episode']==3
 if os.environ.get('JIANCI_EP03_EXECUTE')!='1':print('6 x10 seconds, USD3 base estimate; no API calls');return
 if os.environ.get('GITHUB_RUN_ATTEMPT','1')!='1':raise RuntimeError('No duplicate paid rerun')
 out=Path('output/jianci-ep03-repair');out.mkdir(parents=True,exist_ok=True)
 ...
 base='https://raw.githubusercontent.com/'+os.environ['GITHUB_REPOSITORY']+'/'+os.environ['GITHUB_SHA']+'/video-tests/jianci-ep03-evidence/'
 report={'episode':3,'model':'wan3.0-video','estimate_usd':1,'mode':'one repair each for duplicate question and missing male voice','clips':[]};s.save_report(out,report);pending=[]
 starts=[f'scene-{i:02d}.jpg' for i in range(1,7)]
 refs=[[],[],['scene-02.jpg'],[],[],['scene-05.jpg']]
 for c in cfg['clips']:
  i=int(c['id'])-1
 ...
 s.save_report(out,report)
 if sum('media' in c for c in report['clips'])!=2:raise SystemExit(2)
