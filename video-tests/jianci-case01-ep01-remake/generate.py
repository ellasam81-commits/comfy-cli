...
def config():
    ...
    assert cfg["episode"] == 1 and cfg["duration_seconds"] == 60 and cfg["shots"] == 18
...
def main():
    cfg = config()
    if os.environ.get("JIANCI_EP01_REMAKE_EXECUTE") != "1":
        print("Validated: Episode 01 only, 6 x 10 sec, 18 planned shots, estimated USD 3. No API calls.")
        return
    if int(os.environ.get("GITHUB_RUN_ATTEMPT", "1")) != 1:
        raise RuntimeError("No automatic rerun: could duplicate paid submissions.")
    out = Path("output/jianci-episode01-remake")
