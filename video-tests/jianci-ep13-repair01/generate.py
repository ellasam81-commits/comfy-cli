...
OUT = Path('output/jianci-ep13-repair01')
...
def main():
    ...
    if os.environ.get('JIANCI_EP13_EXECUTE') != '1':
        print('Dry run: first clip only, 10 seconds, 480P, no requests')
        return
    ...
    report = {'episode': 13, 'provider': 'XRToken', 'base_url': host, 'model': cfg['model'],
              'estimate_usd': 0.294, 'max_output_seconds': 10, 'clips': []}
    ...
    save()
    # User explicitly requested a new first segment; submit exactly one 10s clip.
    if not submit(cfg['clips'][0]) or not poll([report['clips'][-1]]):
        raise SystemExit(2)
    print('Episode 13 first segment replacement ready', flush=True)
