...
OUT=Path(f'output/jianci-case3-v1/ep{EP}'+os.environ.get('BATCH',''))
...
def main():
 ...
 def frame(src,t,crop=None):
  ...
  probe=json.loads(subprocess.check_output(['ffprobe','-v','error','-select_streams','v:0','-show_entries','stream=width,height','-of','json',str(dst)]))['streams'][0]
  assert min(probe['width'],probe['height'])>=240,'Reference crop below provider minimum'
   ...
   for row in list(pending):
    try:
     ...
     if row['state']=='succeeded':
      ...
      row['error']=re.sub(r'https?://[^\s\"\']+', '[URL REDACTED]', str(r.get('error',''))[:1500]).replace(key,'[REDACTED]');pending.remove(row);print('FAILED',row['id'],flush=True)
