"""Generate only the approved second episode, with a four-submission hard limit."""
...
def config():
    ...
    assert len(cfg["clips"]) == 4 and [c["id"] for c in cfg["clips"]] == ["03", "04", "05", "06"]
...
def main():
    cfg = config()
    if os.environ.get("JIANCI_EP02_REGEN_EXECUTE") != "1":
        print("Validated: Episode 02 only, 4 x 10 sec replacement clips, estimated USD 2. No API calls.")
        return
    if int(os.environ.get("GITHUB_RUN_ATTEMPT", "1")) != 1:
        raise RuntimeError("No automatic rerun: could duplicate paid submissions.")
    out = Path("output/jianci-episode02-regen")
    ...
    report = {"episode": 2, "model": "wan3.0-video", "estimated_usd": 2, "clips": []}
        ...
        try:
            result = gateway.request(
                "/v2/wan3.0-video",
                {
                    ...
                    "seed": 906207,
                    "negative_prompt": "stairs, staircase, stairwell, steps, risers, level changes, corridor, hallway, balcony, office, desk, open wardrobe, opening doors, moving furniture, text, subtitles, live action, 3D, dissolves, extra cuts, changing faces",
                },
            )
    ...
    print(f"Downloaded {count}/4 clips. No retries submitted.")
    if count != 4:
        raise SystemExit(2)
