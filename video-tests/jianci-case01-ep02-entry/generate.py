"""Generate only the approved second episode, with a one-submission hard limit."""
...
def config():
    ...
    assert cfg["episode"] == 2 and cfg["duration_seconds"] == 60 and cfg["shots"] == 12
    assert len(cfg["clips"]) == 1 and [c["id"] for c in cfg["clips"]] == ["02"]
...
def main():
    cfg = config()
    if os.environ.get("JIANCI_EP02_ENTRY_EXECUTE") != "1":
        print("Validated: Episode 02 only, 1 x 10 sec entry replacement clip, estimated USD 0.50. No API calls.")
        return
    if int(os.environ.get("GITHUB_RUN_ATTEMPT", "1")) != 1:
        raise RuntimeError("No automatic rerun: could duplicate paid submissions.")
    out = Path("output/jianci-episode02-entry")
    ...
    references = [f"https://raw.githubusercontent.com/{repo}/{sha}/video-tests/jianci-transition/modern.png"]
    gateway = shared.Gateway(os.environ.get("SEGMIND_API_KEY"))
    gateway.request("/v1/get-user-credits")
    report = {"episode": 2, "model": "wan3.0-video", "estimated_usd": 0.5, "clips": []}
        ...
        try:
            result = gateway.request(
                "/v2/wan3.0-video",
                {
                    ...
                    "seed": 906218,
                    "negative_prompt": "stairs, staircase, stairwell, stair railing, risers, level changes, sunken room, raised platform, people coming out toward camera, reverse walking, balcony, office, desk, open wardrobe, moving furniture, text, subtitles, live action, 3D, dissolves, extra cuts, changing faces",
                },
            )
    ...
    print(f"Downloaded {count}/1 clips. No retries submitted.")
    if count != 1:
        raise SystemExit(2)
