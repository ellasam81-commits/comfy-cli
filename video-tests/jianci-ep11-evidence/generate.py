...
def main():
 cfg=json.loads((ROOT/'episode.json').read_text());assert len(cfg['clips'])==6 and cfg['episode']==11
 if os.environ.get('JIANCI_EP11_EXECUTE')!='1':print('6 x10 seconds, USD3 base estimate; no API calls');return
 if os.environ.get('GITHUB_RUN_ATTEMPT','1')!='1':raise RuntimeError('No duplicate paid rerun')
 out=Path('output/jianci-ep11-evidence');out.mkdir(parents=True,exist_ok=True)
 ...
 report={'episode':11,'model':'wan3.0-video','estimate_usd':3,'mode':'six new episode11 scenes','clips':[]};s.save_report(out,report);pending=[]
