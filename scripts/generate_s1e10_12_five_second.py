...
def selected_clips(episode: dict[str, Any], clip_code: str | None) -> list[dict[str, Any]]:
    if clip_code is None:
        return episode["clips"]
    selected = [clip for clip in episode["clips"] if clip["id"] == clip_code]
    if len(selected) != 1:
        raise RuntimeError(f"Unknown locked clip: {episode['episode']} {clip_code}")
    return selected


def preflight(episode_code: str | None = None, clip_code: str | None = None) -> None:
    ...
    clip_total = sum(len(selected_clips(episode, clip_code)) for episode in episodes)
    print(f"Preflight passed: {[episode['episode'] for episode in episodes]}, {clip_total} locked clip(s), three shots per clip, 480p Seedance 2 mini, audio + bilingual subtitle QA, no automatic retries.")
...
def generate(episode_code: str | None = None, clip_code: str | None = None, previous_frame: Path | None = None) -> None:
    ...
    previous: Path = previous_frame if previous_frame is not None else style_two
    ...
    try:
        for episode in episodes:
            ...
            for clip in selected_clips(episode, clip_code):
                request_index += 1
                ids = [
...
if __name__ == "__main__":
    ...
    parser.add_argument("--clip", choices=[f"{value:02d}" for value in range(1, 13)])
    ...
    try:
        if args.preflight:
            ...
            preflight(args.episode, args.clip)
        else:
            generate(args.episode, args.clip, args.previous_frame)
    except Exception as exc:
