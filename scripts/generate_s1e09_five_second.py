#!/usr/bin/env python3
"""Generate locked S01E09 five-second clips once each, with no retry path."""
...
ROOT = Path.cwd()
SOURCE = ROOT / "references" / "s1e09"
PLAN_PATH = SOURCE / "five_second_plan.json"
OUTPUT = ROOT / os.environ.get("GENERATION_OUTPUT_DIR", "output/s1e09-gate")
...
LI_KAI_IDENTITY = SOURCE / "identity_refs" / "li_kai.jpg.b64"
IDENTITY_CROPS = {
...
def identity(name: str) -> Path:
    ...
    if name == "li_kai":
        return decode_identity_asset(LI_KAI_IDENTITY, "identity_li_kai.jpg")
...
def load_plan() -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    locked = {
        "episode": "S01E09", "model": "seedance-2.0-mini", "duration_seconds_per_clip": 5,
    ...
    for path in [PLAN_PATH, JIAN_IDENTITY, CHARACTER_CHART, LI_KAI_IDENTITY]:
...
def prepare(clip: dict[str, Any]) -> tuple[list[Path], list[Path]]:
    ...
    identities = [identity(name) for name in clip["characters"] if name != "jiang_lan"]
...
def build_prompt(plan: dict[str, Any], clip: dict[str, Any]) -> str:
    ...
    referenced_characters = [name for name in clip["characters"] if name != "jiang_lan"]
    identity_order = "\n".join(
    ...
    return f"""ORIGINAL SERIES PRODUCTION LOCK. Render exactly one clean five-second 16:9 animated clip for 《吸血法医·剑刺》S01E09《死后的加班》, clip {clip['id']} of 12. Original dark forensic manga-noir only; never imitate a named artist, studio, franchise or copyrighted character.
...
VISUAL / FORENSIC LOCK. {clip['scene']}. Cold blue-black rain-dark 2D manga cinema: clean fine ink linework, restrained cel shading, cinematic light falloff, controlled contrast, stable camera axis, subtle motivated camera movement, natural blink and breath. It must never become a static concept poster, live action, photoreal, 3D or chibi. Evidence is gloved, sealed and procedurally handled. The tablet trigger, revenue share, toxicology, prints and backend trace are independent investigative strands; never declare Li Kai guilty from one clue. Jian Ci's vampire physiology never proves a case. Clip 01 must match-cut directly from S01E08's wet desk, black laptop and red live-status dot, then proceed inside the same stream room; do not create a literal split screen or repeat a prior video inside the frame.
...
def main() -> None:
    ...
    if clips[0]["id"] != "01" and not os.environ.get("PREVIOUS_VIDEO_URL"):
        raise RuntimeError("A later S01E09 clip batch needs PREVIOUS_VIDEO_URL; no paid request made")
    previous: Path | None = None
    manifest: dict[str, Any] = {"episode": "S01E09", "clips": [], "request_count": 0, "automatic_retries": 0, "status": "running", "started_at": stamp(), "plan_sha256": sha(PLAN_PATH)}
    ...
    try:
        for clip in clips:
            ...
            video = RAW / f"S1E09_clip_{clip['id']}_raw.mp4"
