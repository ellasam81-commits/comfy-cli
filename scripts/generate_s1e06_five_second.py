#!/usr/bin/env python3
"""Generate locked S01E06 five-second clips once each, with no retry path."""
...
OUTPUT = ROOT / os.environ.get("GENERATION_OUTPUT_DIR", "output/s1e06-gate")
...
TAO_WEN_IDENTITY = SOURCE / "identity_refs" / "tao_wen.jpg.b64"
SHEN_ZHOU_IDENTITY = SOURCE / "identity_refs" / "shen_zhou.jpg.b64"
IDENTITY_CROPS = {
...
def decode_panel(clip_id: str, index: int) -> Path:
    """Decode one of the four locked storyboard panels before Segmind upload."""
...
def decode_identity_asset(encoded: Path, target_name: str) -> Path:
    """Decode a committed character identity image without storing binary blobs."""
    target = RUNTIME / target_name
    if not encoded.is_file():
        raise RuntimeError(f"Missing locked identity: {encoded}")
    try:
        target.write_bytes(b64decode(encoded.read_text(encoding="ascii"), validate=True))
    except Exception as exc:
        raise RuntimeError(f"Invalid locked identity: {encoded}") from exc
    image_ok(target)
    return target
...
def identity(name: str) -> Path:
    ...
    if name == "tao_wen":
        return decode_identity_asset(TAO_WEN_IDENTITY, "identity_tao_wen.jpg")
    if name == "shen_zhou":
        return decode_identity_asset(SHEN_ZHOU_IDENTITY, "identity_shen_zhou.jpg")
    # Shen Mi is only reconstructed tactfully from storyboard panels in S01E06;
    # the production does not create or reuse a deceased-person glamour image.
    if name == "shen_mi":
        raise RuntimeError("Shen Mi is locked to the clip storyboard panels only")
...
def load_plan() -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    locked = {
        "episode": "S01E06", "model": "seedance-2.0-mini", "duration_seconds_per_clip": 5,
        "clip_count": 12, "shots_per_clip": 4, "resolution": "480p", "aspect_ratio": "16:9",
    }
    ...
    for clip in clips:
        if len(clip.get("shots", [])) != 4 or not isinstance(clip.get("dialogue"), list):
            raise RuntimeError(f"Clip {clip.get('id')} is not a four-shot locked clip")
    ...
    for path in [PLAN_PATH, JIAN_IDENTITY, CHARACTER_CHART, TAO_WEN_IDENTITY, SHEN_ZHOU_IDENTITY]:
        image_ok(path) if path.suffix.lower() in {".jpg", ".jpeg", ".png"} else None
    return plan, clips


def prepare(clip: dict[str, Any]) -> tuple[list[Path], list[Path]]:
    panels = [decode_panel(clip["id"], index) for index in range(1, 5)]
    identities = [identity(name) for name in clip["characters"] if name not in {"shen_mi", "jiang_lan"}]
    for path in identities:
        image_ok(path)
    if "jian_ci" in clip["characters"] and JIAN_IDENTITY not in identities:
...
def build_prompt(plan: dict[str, Any], clip: dict[str, Any]) -> str:
    ...
    audio_instruction = (
        "Generate synchronized clear native Mandarin audio with restrained room tone and a low subtle industrial pulse. "
        "Speak only the exact Chinese line(s) below, clearly above ambience, without overlap, English, invented speech or silence:\n"
        + speech
        if speech else
        "Generate only restrained room tone, rain/monitor ambience and a low subtle industrial pulse. "
        "No dialogue, narration, words, singing, whispering or other vocalisation."
    )
    profiles = "\n".join(plan["character_lock"].split(". "))
    referenced_characters = [name for name in clip["characters"] if name not in {"shen_mi", "jiang_lan"}]
    identity_order = "\n".join(
        f"Identity reference {index + 5}: {name}."
        for index, name in enumerate(referenced_characters)
    ) or "No additional face identity image: follow the locked storyboard panels exactly."
    return f"""ORIGINAL SERIES PRODUCTION LOCK. Render exactly one clean five-second 16:9 animated clip for 《吸血法医·剑刺》S01E06《被数字选中的凶手》, clip {clip['id']} of 12. Original dark forensic manga-noir only; never imitate a named artist, studio, franchise or copyrighted character.

REFERENCE ORDER IS LOCKED. References 1-4 are the four storyboard shot authorities. They define composition, character positions, action and handed props. Following images are identity authorities only:
{identity_order}
...
RENDER EXACTLY FOUR FULL-SCREEN CINEMATIC SHOTS:
{shots}
...
def verify(video: Path, clip: dict[str, Any], model: Any) -> dict[str, Any]:
    ...
    if clip["dialogue"] and (sum(item["end"] - item["start"] for item in speech) < 0.4 or len(re.findall(r"[\u3400-\u9fff]", transcript)) < 2):
    ...
    dump(AUDIT / f"clip_{clip['id']}_speech.json", {"language": getattr(info, "language", None), "segments": speech, "transcript": transcript, "expected": [line["zh"] for line in clip["dialogue"]]})
    return {"duration": duration, "width": visual[0].get("width"), "height": visual[0].get("height"), "audio_codec": audio[0].get("codec_name"), "max_volume_db": float(level.group(1)), "asr": transcript, "speech_segments": speech}
...
def main() -> None:
    ...
    if clips[0]["id"] != "01" and not os.environ.get("PREVIOUS_VIDEO_URL"):
        raise RuntimeError("A later S01E06 clip batch needs PREVIOUS_VIDEO_URL; no paid request made")
    previous: Path | None = None
    manifest: dict[str, Any] = {"episode": "S01E06", "clips": [], "request_count": 0, "automatic_retries": 0, "status": "running", "started_at": stamp(), "plan_sha256": sha(PLAN_PATH)}
    ...
    try:
        for clip in clips:
            ...
            video = RAW / f"S1E06_clip_{clip['id']}_raw.mp4"
