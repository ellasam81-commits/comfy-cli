#!/usr/bin/env python3
"""Assemble verified S01E09 clips with a header and voice-synchronous bilingual subtitles."""
...
ROOT = Path.cwd()
PLAN = ROOT / "references/s1e09/five_second_plan.json"
RAW = ROOT / "output/s1e09-all/raw"
AUDIT = ROOT / "output/s1e09-all/audit"
OUT = ROOT / "output/s1e09-final"
...
def main() -> None:
    ...
    if [clip["id"] for clip in clips] != [f"{value:02d}" for value in range(1, 13)]:
        raise RuntimeError("S01E09 plan must retain the 12 locked clips")
    ...
    for clip in clips:
        source = RAW / f"S1E09_clip_{clip['id']}_raw.mp4"
        if not source.is_file():
    ...
    offset = 0.0
    lines = [
        "[Script Info]", "Title: S01E09 Dead After Hours", "ScriptType: v4.00+", f"PlayResX: {WIDTH}", f"PlayResY: {HEIGHT}", "WrapStyle: 2", "ScaledBorderAndShadow: yes", "",
        "[V4+ Styles]",
    ...
    total = sum(durations)
    header = "《吸血法医·剑刺》｜S01E09《死后的加班》｜制作：林爱丽"
    ...
    for clip, clip_duration in zip(clips, durations):
        ...
        if len(cue_list) != len(clip["dialogue"]):
            raise RuntimeError(f"S01E09 voice-cue count must match dialogue count for clip {clip['id']}")
        for spoken, cue in zip(clip["dialogue"], cue_list):
            ...
            if not 0 <= local_start < local_end <= clip_duration:
                raise RuntimeError(f"Invalid S01E09 voice cue for clip {clip['id']}: {cue}")
    ...
    ass = OUT / "S1E09_bilingual.ass"
    ass.write_text("\n".join(lines) + "\n", encoding="utf-8")

    concat = OUT / "concat.txt"
    concat.write_text("\n".join(f"file '{(NORMALIZED / f'{clip['id']}.mp4').as_posix()}'" for clip in clips) + "\n", encoding="utf-8")
    joined = OUT / "S1E09_DeadAfterHours_joined.mp4"
    pending_joined = OUT / "S1E09_DeadAfterHours_joined.pending.mp4"
    run(["ffmpeg", "-hide_banner", "-loglevel", "error", "-y", "-f", "concat", "-safe", "0", "-i", str(concat), "-c", "copy", "-movflags", "+faststart", str(pending_joined)])
    playable_with_audio(pending_joined, total, "Joined S1E09")
    pending_joined.replace(joined)
    final = OUT / "S1E09_DeadAfterHours_Final.mp4"
    pending_final = OUT / "S1E09_DeadAfterHours_Final.pending.mp4"
    run([
    ...
    playable_with_audio(pending_final, total, "Final S1E09")
    ...
    package = OUT / "S1E09_DeadAfterHours_Final.zip"
    pending_package = OUT / "S1E09_DeadAfterHours_Final.pending.zip"
